...
I created a scope method called `priority_order` that used the `order` method to sort all my tasks on a descending scale from 1-6. This `order` method is an Active Record finder method that retrieves objects from the database.  All finder methods let you pass arguments into it to execute specific queries in your database without having to write out the lengthy SQL code. I specifically chose the `order` method to pull out records in a certain order from the database. Here we have 6 records we are retrieving and want to organize them in descending order, hence the `desc` specification seen below in my code snippet.  With this new scope method, a user can rank their Daily Tasks from Highest to Lowest Priority. Where 6 is the highest priority and 1 is the lowest priority. Below is how I implemented my custom scope method, `priority_order`.
